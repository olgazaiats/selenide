...
public class OpenEnglishTest extends SelenideTestBase{
    ...
    @Test
    public void recoverEmailValidationTest() throws InterruptedException {
        open(loginURL);
        ...
        $(By.xpath("//div[@class=\"tooltipster-content\"]")).shouldHave(text("El email que usted ingresó no ha sido reconocido. Por favor, inténtelo de nuevo."));
        recoverPasswordPage.removeEmail();

        //Invalid email (no @) validation
        recoverPasswordPage.enterEmail("olga.com").submitEmail();
        $(By.xpath("//div[@class=\"tooltipster-content\"]"))
